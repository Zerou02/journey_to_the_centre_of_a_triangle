...
func drawCartesianLine(cp1, cp2 glm.Vec2, ctx *closedGL.ClosedGLContext, depth int, c glm.Vec4) {
	ctx.DrawLine(CartesianToSSPoint(cp1, ctx.Window.Wh), CartesianToSSPoint(cp2, ctx.Window.Wh), c, c, depth)
}
